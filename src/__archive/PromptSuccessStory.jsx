  ...
  const successStories = [
    {
      ...
      additionalTasks: [
        {
          id: 'task1',
          title: '예시 활용: 고급 모드에서 구체적인 사례를 추가하면 더 정확한 결과를 얻을 수 있습니다',
          content: '구체적인 사례를 포함하면 AI가 더 정확하게 이해합니다. 예를 들어, "온라인 쇼핑몰 고객 응대" 대신 "의류 쇼핑몰에서 사이즈 문의하는 고객 응대"처럼 구체적으로 작성해보세요.'
        },
        {
          id: 'task2',
          title: '반복 개선: 첫 결과가 아쉬우면 세부 조금씩 바꿔보세요',
          content: '한 번에 완벽한 결과를 기대하기보다는, 결과를 보고 조금씩 수정해가며 원하는 방향으로 개선해 나가세요. 톤, 길이, 스타일 등을 하나씩 조정해보면 좋습니다.'
        }
      ]
    },
    {
      ...
      additionalTasks: [
        {
          ...
          content: '독자의 관심을 끌고 유지하려면 명확한 시작, 중간, 끝이 있어야 합니다. 문제 제기 → 갈등 고조 → 해결 과정 → 결론의 흐름을 만들어보세요.'
        },
        {
          id: 'task4',
          title: '감정 연결: 독자가 공감할 수 있는 감정적 요소를 추가하세요',
          content: '단순한 정보 전달보다는 독자가 감정적으로 연결될 수 있는 요소를 포함시키면 더 강력한 스토리가 됩니다. 개인적 경험이나 구체적인 사례를 활용해보세요.'
        }
      ]
    }
  ];
        ...
        <div className="space-y-6">
          {successStories.map((story) => (
            <div 
              ...
              style={{
                background: `linear-gradient(to bottom right, ${story.bgColor === 'bg-blue-50' ? 'rgb(239 246 255)' : 'rgb(250 245 255)'}, white)`
              }}
            >
              <div className="flex items-start space-x-4">
                <div className={`w-12 h-12 rounded-full ${story.iconBgColor} flex items-center justify-center flex-shrink-0`}>
                  <span className="text-2xl">{story.icon}</span>
                </div>
                <div className="flex-1">
                  <h3 className="font-semibold text-lg text-gray-900 mb-2">{story.type}</h3>
                  <p className="text-gray-600 mb-4">{story.description}</p>
                  
                  {/* 추가 작업 섹션 */}
                  {story.additionalTasks && story.additionalTasks.length > 0 && (
                    <div className="mt-6 space-y-3">
                      <h4 className="font-medium text-gray-700 mb-3">추가 작업</h4>
                          ...
                          <div className="relative">
                            ...
                            {expandedItems[task.id] && (
                              <div className="mt-3 ml-8 animate-fadeInUp">
                                <div className={`p-4 ${story.toggleBgColor} rounded-lg border ${story.toggleBorderColor}`}>
                  ...
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-700">
                      💡 <strong>생성된 프롬프트를 복사해서 AI 도구에 붙여넣으세요</strong>
                    </p>
                  </div>
                </div>
              </div>
