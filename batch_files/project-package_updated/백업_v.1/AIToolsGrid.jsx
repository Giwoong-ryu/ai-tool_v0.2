    ...
    import AIToolIcon from './AIToolIcon'; // AIToolIcon은 이제 자체적으로 아이콘을 처리합니다.
    ...
    const PromptGeneratorModal = ({ isOpen, onClose, aiToolsData }) => {
      ...
      const [tone, setTone] = useState('none'); // 초기값을 'none'으로 설정하여 Select 오류 방지
      ...
      // 추천 모델 URL 맵 (AIToolsGrid.jsx 내에서 사용)
      ...
      const handleGeneratePrompt = useCallback(async () => {
        ...
        const tonePart = tone === 'none' ? '' : `톤: ${tone}`; // 'none'일 경우 비어있는 문자열로 처리
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={tone} onValueChange={setTone}>
                    ...
                    <SelectContent>
                      <SelectItem value="none">선택 안 함</SelectItem> {/* value를 "none"으로 변경 */}
                ...
                {selectedTool && (
                  <div className="flex items-center gap-3">
                    <div className="flex-shrink-0 w-12 h-12 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                      {/* 여기서는 여전히 AIToolIcon 컴포넌트를 사용하지 않고, Lucide.Globe로 대체합니다. */}
                      {React.createElement(Lucide.Globe, { className: "w-6 h-6 text-gray-700 dark:text-gray-200" })}
                    </div>
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        {selectedTool.usecases.map((usecase, index) => (
                          <li key={index} className="text-sm">
                            <span className="font-medium text-primary dark:text-blue-300">{usecase.title}:</span>{' '}
                            {usecase.detail} ({usecase.example})
                          </li>
                        ))}
                      </ul>
                    </div>
